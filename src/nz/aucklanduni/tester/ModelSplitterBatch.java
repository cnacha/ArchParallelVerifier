...
public class ModelSplitterBatch {
	...
	private static final String testName = "system";

	private static final String inputfile = "output/system/models.txt";//"input/" + testName + ".txt";
	private static final String outputModelfile = "input/"+testName+"/" + testName + "-submodel";
	private static final String outputResultfile = "result/" + testName + "-split.csv";
		...
		try {
			...
			for (int i = 0; i < overviewModel.getComponents().size(); i++) {
				if (overviewModel.getComponents().get(i).getCalls() != null
					...
					VerifierOnThreadedService verifier = new VerifierOnThreadedService(overviewModel.getComponents(),80 , true);
					verifier.start();
			...
			List<SubModel> modelToRemove = new ArrayList<SubModel>();
			for (SubModel mod : submodelList) {
				mod.clean();
				//mod.cleanNonCaller();
				//if(mod.getComponents().size()==0) {
				//	modelToRemove.add(mod);
				//}
				mod.checkAndSetStart();
				
				
				System.out.println("Finish check and set start...");
			}
			
			submodelList.removeAll(modelToRemove);
			...
			overviewResult.write((count-1)+",elapse:"+ ((new Date()).getTime() - startTime) +"\n");
