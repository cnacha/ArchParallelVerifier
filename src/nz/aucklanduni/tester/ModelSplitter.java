...
public class ModelSplitter {
	...
	private static final int MAX_COMPONENT = 8;

	private static final String testName =  "ec-model";
...
//	private static final String filename = "input/test-model.txt";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				ComponentConfig comp = gson.fromJson(line,ComponentConfig.class);
			...
			// find sub-models
			//List<List<ComponentConfig>> moduleList = new ArrayList<List<ComponentConfig>>();
		...
		/*	for(int i=0;i<1; i++) {
				tmpModels = overlapModel(compList2, tmpModels);
				overlapList.addAll(tmpModels);
			}
		*/
			...
			/*for(int i=0; i<submodelList.size();i++) {
				for(int j=0; j<submodelList.size(); j++) {
					if(i!=j) {
						// match calls array with id in the module list by j
						List<ComponentConfig> ol = overlapComp(compList2,submodelList.get(i).getComponents(),submodelList.get(j).getComponents() );
						if(ol.size()>0) {
							SubModel mod = new SubModel(modId,ol);
							mod.clean();
							overlapList.add(mod);
							modId++;
						}
			...
			for(int i=0; i<overviewModel.getComponents().size();i++) {
				if(overviewModel.getComponents().get(i).getCalls()!=null && overviewModel.getComponents().get(i).getCalls().length>0) {
					...
					VerifierOnThreadedService verifier = new VerifierOnThreadedService(overviewModel.getComponents(), 80+i, true);
					verifier.start();
	...
	private static List<ComponentConfig> overlapComp(Hashtable<Integer,ComponentConfig> compList,List<ComponentConfig> a, List<ComponentConfig> b) throws CloneNotSupportedException{
	...
	private static List<ComponentConfig> collectComp(Hashtable<Integer,ComponentConfig> compList, List<ComponentConfig> result,int startId,int numOfComponent){
				...
				if(comp.getCalls()!=null) {
					...
					for(int i=0; i<comp.getCalls().length; i++) {
						if(numOfComponent>0) {
							...
							List<ComponentConfig> resub = collectComp(compList, result, comp.getCalls()[i], numOfComponent);
